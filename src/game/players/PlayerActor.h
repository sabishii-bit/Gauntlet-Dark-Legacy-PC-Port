...
class PlayerActor {
public:
    static constexpr float kMinSpeed = 5.0f;  ///< units per second with no speed at all
    static constexpr float kMaxSpeed = 12.5f; ///< units per second at a speed stat of 1000
    static constexpr float kStatScale = 0.001f;
    static constexpr float kMoveLimit = 1.5f; ///< at most this many speeds of travel per second
    static constexpr float kStepUp = 1.5f;    ///< the highest ledge walked up
    static constexpr float kDrop = 3.0f;      ///< the deepest drop walked down
    static constexpr float kFootClearance = 0.2f;
    static constexpr float kDefaultHeight = 5.0f;
    static constexpr float kDefaultWidth = 1.5f;
    static constexpr float kDefaultFollowHeight = 2.5f;
     ...
     * where it is. */
    void update(const MoveInput& input, float cameraYaw, float seconds,
                const WorldCollision* collision, float moveScale = 1.0f, bool keepFacing = false);
    /** The way `input` would send the character, as a heading about the upright. */
    static float headingOf(const MoveInput& input, float cameraYaw);
    ...
    float speed() const { return m_speed + m_paceBonus; }
    /** Units a second a speed powerup adds to the class's pace. */
    void setPaceBonus(float bonus) { m_paceBonus = bonus; }
    ...
    float reach() const { return m_radius * 2.0f; }
...
private:
    ...
    float m_yaw = 0.0f;
    float m_speed = kMinSpeed;
    float m_paceBonus = 0.0f;
    float m_radius = kDefaultWidth * 0.5f;
    float m_height = kDefaultHeight;
    float m_followHeight = kDefaultFollowHeight;
    bool m_moving = false;
};
