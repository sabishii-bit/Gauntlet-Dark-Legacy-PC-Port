...
void TurboMove::advance(PlayerAnimator::Action action, float frame, const Vec3& facing,
    ...
    m_weaponHidden = false;
    for (std::size_t slot = 0; slot < m_all.size(); ++slot) {
        const MoveStrike& row = stats->moveStrikes[static_cast<std::size_t>(m_all[slot])];
...
void TurboMove::volley(std::size_t slot, const MoveStrike& strike, float frame, const Vec3& facing,
                       const Events& events) {
    ...
    while (m_volleysShot[slot] < due) {
        ...
        if ((strike.flags & (MoveStrike::kSweepsIn | MoveStrike::kSweepsOut)) != 0) {
            float through = span > 0.0f ? std::clamp(shotAt / span, 0.0f, 1.0f) : 1.0f;
            if ((strike.flags & MoveStrike::kSweepsIn) != 0) {
                through = 1.0f - through;
            }
            angle *= through;
        }
        const float heading = std::atan2(facing.x, facing.z) + angle;
