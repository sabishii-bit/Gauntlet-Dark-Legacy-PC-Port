...
struct LevelRef {
    ...
    int realmId = -1;      ///< the original's world id for the realm
    ...
    static constexpr int kTowerRealm = 13;
    ...
    static int orderOf(int realmId);
...
public:
    ...
    struct Realm {
        ...
        std::vector<int> runes; ///< the runestone each level holds, from one; nought for none
    };
    ...
    std::vector<int> runesOf(std::string_view realmFile) const;
