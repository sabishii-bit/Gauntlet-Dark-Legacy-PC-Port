...
public:
    ...
    static constexpr std::int32_t kWelcomeIndex = 3; ///< greeting a player who steps up to him
    static constexpr std::int32_t kGoAwayIndex = 4;  ///< seeing them off once they are done
    static constexpr std::int32_t kGestureIndex = 6; ///< the sweep towards the crystals
    static constexpr std::uint32_t kLookout = 0;     ///< the event marker parameter naming his spot
    ...
    std::uint32_t sequence() const { return m_player.sequence(); }
    ...
    bool playing(std::int32_t index) const {
        return index >= 0 && static_cast<std::size_t>(index) < m_sequences.size() &&
               m_sequences[static_cast<std::size_t>(index)] >= 0 &&
               sequence() ==
                   static_cast<std::uint32_t>(m_sequences[static_cast<std::size_t>(index)]);
    }

private:
    std::uint32_t sequenceFor(std::int32_t index) const;

    TreeModel m_model;
    const TreeInfo* m_tree = nullptr;
    std::array<std::int32_t, kSequences.size()> m_sequences{-1, -1, -1, -1, -1, -1, -1};
