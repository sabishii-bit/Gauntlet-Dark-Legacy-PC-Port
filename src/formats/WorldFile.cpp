...
constexpr usize kTriangleSize = 40;
 ...
 */
Vec3 planeOffset(const Vec3& normal, f32 x, f32 z) {
    const f32 tilt = std::acos(std::clamp(normal.y, -1.0f, 1.0f));
    const f32 turn =
        (normal.x == 0.0f && normal.z == 0.0f) ? 0.0f : std::atan2(-normal.x, -normal.z);
    const f32 sinTilt = std::sin(tilt);
    const f32 cosTilt = std::cos(tilt);
    const f32 sinTurn = std::sin(turn);
    const f32 cosTurn = std::cos(turn);
...
std::string readFixedName(ByteReader& reader, usize width) {
    std::string name;
...
ItemInfoRecord readItemInfo(std::span<const u8> record) {
    ByteReader reader(record);
    ItemInfoRecord out;
    out.type = static_cast<s32>(reader.readU32());
    out.subtype = static_cast<s32>(reader.readU32());
    out.collisionType = static_cast<s16>(reader.readU16());
    out.collisionFlags = static_cast<s16>(reader.readU16());
    ...
    out.value = static_cast<s16>(reader.readU16());
    out.armor = static_cast<s16>(reader.readU16());
    out.hitPoints = static_cast<s16>(reader.readU16());
...
ItemInstanceRecord readItemInstance(std::span<const u8> record) {
    ByteReader reader(record);
    ItemInstanceRecord out;
    out.info = static_cast<s16>(reader.readU16());
    out.minPlayers = static_cast<s8>(reader.readU8());
    out.flags = reader.readU8();
    out.triangleIndex = static_cast<s16>(reader.readU16());
    out.triangleCount = static_cast<s16>(reader.readU16());
    out.name = readFixedName(reader, kItemNameSize);
    out.position = readVec3(reader);
    out.rotation = readVec3(reader);
    for (u8& value : out.params) {
        value = reader.readU8();
    }
    return out;
}
...
WorldFile WorldFile::parse(std::span<const u8> bytes) {
    ...
    const u32 triangleOffset = header.readU32();
    ...
    if (world.particleSystemCount > 0 && particleOffset != 0) {
        checkRange(bytes, particleOffset, world.particleSystemCount,
                   ParticleTemplateRecord::kSize, "particle templates");
        for (u32 i = 0; i < world.particleSystemCount; ++i) {
            world.particles.push_back(readParticleTemplate(bytes.subspan(
                particleOffset + usize{i} * ParticleTemplateRecord::kSize,
    ...
    if (world.animationCount > 0 && keyHeaderOffset != 0) {
        ...
        for (u32 i = 0; i < world.animationCount; ++i) {
            ByteReader reader(
                bytes.subspan(animationOffset + usize{i} * kAnimationSize, kAnimationSize));
            WorldAnimationRecord animation;
            animation.objectIndex = static_cast<s16>(reader.readU16());
            animation.frameCount = static_cast<s16>(reader.readU16());
            reader.readU16(); // relocation mark
            animation.state = reader.readU16();
            animation.startFrame = readF32(reader);
            const u32 entry = reader.readU32();
            animation.track =
    ...
    if (world.itemInfoCount > 0) {
        checkRange(bytes, itemInfoOffset, world.itemInfoCount, ItemInfoRecord::kSize,
                   "item infos");
        for (u32 i = 0; i < world.itemInfoCount; ++i) {
            world.itemInfos.push_back(readItemInfo(bytes.subspan(
                itemInfoOffset + usize{i} * ItemInfoRecord::kSize, ItemInfoRecord::kSize)));
        }
    }
    if (world.itemInstanceCount > 0) {
        checkRange(bytes, itemInstanceOffset, world.itemInstanceCount, ItemInstanceRecord::kSize,
                   "item instances");
        for (u32 i = 0; i < world.itemInstanceCount; ++i) {
            world.itemInstances.push_back(readItemInstance(
                bytes.subspan(itemInstanceOffset + usize{i} * ItemInstanceRecord::kSize,
    ...
    for (u32 i = 0; i < world.collisionTriangleCount; ++i) {
        ByteReader reader(
            bytes.subspan(triangleOffset + usize{i} * kTriangleSize, kTriangleSize));
        ...
        const auto v1x = static_cast<s16>(reader.readU16());
        const auto v1z = static_cast<s16>(reader.readU16());
        const auto v2x = static_cast<s16>(reader.readU16());
        const auto v2z = static_cast<s16>(reader.readU16());
        triangle.vertices[1] =
            triangle.vertices[0] + planeOffset(triangle.normal, static_cast<f32>(v1x) * kOffsetUnit,
                                               static_cast<f32>(v1z) * kOffsetUnit);
        triangle.vertices[2] =
            triangle.vertices[0] + planeOffset(triangle.normal, static_cast<f32>(v2x) * kOffsetUnit,
                                               static_cast<f32>(v2z) * kOffsetUnit);
