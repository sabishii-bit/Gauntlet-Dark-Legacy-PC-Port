...
ItemInfoRecord readItemInfo(std::span<const u8> record) {
    ...
    if (out.type == ItemInfoRecord::kChoiceList) {
        ...
        const auto count =
            std::min(static_cast<usize>(std::max(out.subtype, 0)), ItemInfoRecord::kMostChoices);
...
WorldFile WorldFile::parse(std::span<const u8> bytes) {
    ...
    if (world.particleSystemCount > 0 && particleOffset != 0) {
        ...
        for (u32 i = 0; i < world.particleSystemCount; ++i) {
            world.particles.push_back(readParticleTemplate(
                bytes.subspan(particleOffset + usize{i} * ParticleTemplateRecord::kSize,
    ...
    for (u32 i = 0; i < world.collisionTriangleCount; ++i) {
        ByteReader reader(bytes.subspan(triangleOffset + usize{i} * kTriangleSize, kTriangleSize));
