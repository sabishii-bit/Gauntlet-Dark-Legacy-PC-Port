...
CritterFile parseCritterWad(std::span<const u8> bytes) {
    ...
    CritterFile file;
    file.sounds =
        readRecords<CritterSoundRecord>(bytes, sections, "SFXX", kSoundSize, [&](usize at) {
    ...
    file.damages =
        readRecords<CritterDamageRecord>(bytes, sections, "DAMG", kDamageSize, [&](usize at) {
    ...
    file.descriptors = readRecords<CritterDescriptorRecord>(
        bytes, sections, "DESC", kDescriptorSize, [&](usize at) {
