...
struct TextureSetEntry {
    ...
    std::uint32_t frames =
        0; ///< frames of the animation starting here (consecutive entries), 0 = none
    ...
    bool clampV = false;
    bool noPicture = false; ///< the archive keeps none for it: it is drawn clear         ///< and
                            ///< down; a texture may tile one way only
    std::filesystem::path file;

    static constexpr std::uint32_t kExternal = 0x20; ///< the flag of textures another archive holds
    static constexpr std::uint32_t kHasAlpha = 0x80; ///< the flag of textures drawn blended
...
public:
    ...
    const TextureSetEntry& entry(std::uint32_t index) const;
    ...
    const Image& image(std::uint32_t index);

    /** GPU texture, created on first use; throws like image(). */
    const Texture& texture(RenderDevice& device, std::uint32_t index);
