...
std::array<T, 4> readFour(const Json& entry, const char* key, std::array<T, 4> fallback) {
    ...
    for (std::size_t i = 0; i < fallback.size() && i < values.size(); ++i) {
        fallback[i] = values[i];
    }
    return fallback;
}
...
ParticleTemplate readParticleTemplate(const Json& entry) {
    ...
    const auto emitterLife = entry.value("emitterLife", std::vector<float>{0.0f, 0.0f});
    const auto particleLife = entry.value("particleLife", std::vector<float>{0.0f, 0.0f});
    ...
    particle.rate = readFour<float>(entry, "rate", particle.rate);
    ...
    particle.rgba = readFour<std::uint32_t>(entry, "rgba", particle.rgba);
    particle.width = readFour<float>(entry, "width", particle.width);
