...
class ModelSprite {
public:
    static constexpr float kDepth = 0.75f;      ///< canvas depth the model is centred on
    static constexpr float kDepthRange = 0.02f; ///< depth per model unit, nearer is larger
     ...
     */
    void draw(Canvas& canvas, Vec2 position, float scale, float pitch,
