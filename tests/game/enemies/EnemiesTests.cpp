...
std::filesystem::path unpackedRoot() {
    return test::unpackedOrSkip("MONSTERS/GRU/animations.json").parent_path().parent_path().parent_path();
}
...
TEST_CASE("a grunt is bred ahead of its generator, chases the player it sees and strikes on touch",
    ...
    const int closing = stepsUntil(
        enemies, party, [&] { return glm::distance(enemies.positionOf(*id), party[0].position) < 3.5f; },
        600);
    REQUIRE(closing < 600);
    REQUIRE(glm::distance(enemies.positionOf(*id), party[0].position) < glm::distance(from, party[0].position));
...
TEST_CASE("a grunt gets round a wall between it and its player, stops at a ledge, and does "
    ...
    const int steps = stepsUntil(
        ...
        [&] {
            ...
            bumped = bumped || (enemies.positionOf(*id).z > 17.0f && enemies.positionOf(*id).z < 20.0f);
    ...
    REQUIRE(stepsUntil(enemies, party, [&] { return enemies.targetOf(*id) == 0 && glm::distance(enemies.positionOf(*id), party[0].position) < 4.0f; }, 1200) < 1200);
...
TEST_CASE("the swarm is found by missiles, sweeps and strikes, is capped, and sleeps until woken",
    ...
    REQUIRE_FALSE(enemies.struckBy(Vec3{-5.0f, 3.0f, -5.0f}, Vec3{-5.0f, 3.0f, 30.0f}, 0.5f).has_value());
    ...
    REQUIRE(enemies.reachedBy(Vec3{0.0f, 0.0f, 0.0f}, 12.0f, 3.2f, Vec3{0.0f, 0.0f, 1.0f}).size() == 2);
