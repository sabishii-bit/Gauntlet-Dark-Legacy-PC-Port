...
TEST_CASE("a boss meter fills its two strips by the original's arithmetic and eases toward "
    ...
    REQUIRE(meter.fillWidths() == std::array<int, 2>{0, 0});
    ...
    REQUIRE(meter.fillWidths() == std::array<int, 2>{256, 256 - 53});
    ...
    REQUIRE(meter.fillWidths() == std::array<int, 2>{44, 0}); // the cap is not fill
    ...
    meter.update(2, 3000.0f, 3000.0f, true, false);
    REQUIRE(meter.fillWidths() == std::array<int, 2>{256 - 44 - 53, 0});
}
