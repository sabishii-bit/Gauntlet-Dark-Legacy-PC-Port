...
void unpackWorld(const AssetLocator& files, const std::filesystem::path& outDir, Summary& summary) {
...
void unpackClassData(const std::filesystem::path& file, const std::filesystem::path& outDir,
    ...
    json.endArray();
    json.key("familiarShotOffset").beginArray();
    for (const f32 axis : record.familiarShotOffset) {
        json.value(axis);
    }
    json.endArray();
...
void unpackWorldData(const std::filesystem::path& file, const std::filesystem::path& outDir,
    ...
    for (const LevelRecord& level : data.levels) {
        ...
        json.key("timeLimit").value(level.timeLimit);
